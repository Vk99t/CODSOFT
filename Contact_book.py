...
def add_contact():
    ...
    if name and phone.isdigit() and len(phone)== 10 and is_valid_email(email):  # Check if phone number has at least 10 digits
        contact = {"Name": name, "Phone": phone, "Email": email, "Address": address}
        contacts.append(contact)
        clear_entries()
        update_contact_list()
    else:
        messagebox.showerror("Error", "Name is required, and Phone must have at least 10 digits! and Email must have a proper format!")
...
def search_contact():
    ...
    for contact in found_contacts:
        ...
        if search_term:  # Highlight matching results with a color (customize as needed)
            contact_listbox.itemconfig(tk.END, {'bg': 'yellow'})
        else:
            contact_listbox.itemconfig(tk.END, {'bg': 'white'})
